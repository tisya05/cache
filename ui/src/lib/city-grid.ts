...
export type BuildingDef = {
  ...
  onChainKind: number;
  /**
   * Uniform scale applied when rendering on the grid (not in the shop list).
   * The raw sprite art is much narrower than its ground tile -- e.g. house is
   * 185px on a 282px tile -- which left visible bare ground around every
   * placed building. Hand-tuned per building rather than one formula: a
   * blind "scale to 90% of tile width" blows narrow/tall sprites (tree,
   * tower) up into oversized blobs since their height scales with their
   * width.
   */
  renderScale: number;
};
...
export const BUILDING_CATALOG: BuildingDef[] = [
  { key: "tree", label: "Tree", cost: 1, sprite: "/tiles/sprites/tree.png", width: 79, height: 134, onChainKind: 0, renderScale: 1.5 },
  { key: "house", label: "House", cost: 3, sprite: "/tiles/sprites/house.png", width: 185, height: 159, onChainKind: 1, renderScale: 1.4 },
  { key: "shop", label: "Shop", cost: 5, sprite: "/tiles/sprites/shop.png", width: 199, height: 216, onChainKind: 2, renderScale: 1.35 },
  { key: "park", label: "Park", cost: 6, sprite: "/tiles/sprites/park.png", width: 267, height: 166, fullTile: true, onChainKind: 3, renderScale: 1.0 },
  { key: "apartment", label: "Apartment", cost: 10, sprite: "/tiles/sprites/apartment.png", width: 181, height: 254, onChainKind: 4, renderScale: 1.35 },
  { key: "tower", label: "Tower", cost: 20, sprite: "/tiles/sprites/tower.png", width: 123, height: 266, onChainKind: 5, renderScale: 1.6 },
];
...
export function loadPlacedBuildings(): PlacedBuilding[] {
  return readJSON<PlacedBuilding[]>(GRID_KEY, []);
}
...
export function loadBuildLog(): number[] {
  return readJSON<number[]>(BUILD_LOG_KEY, []);
}
